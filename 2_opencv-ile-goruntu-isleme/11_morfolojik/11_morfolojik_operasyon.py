# -*- coding: utf-8 -*-
"""
Created on Sun May 16 17:56:31 2021
...
plt.figure(), plt.imshow(noise_img, cmap = "gray"), plt. axis("off"), plt.title("ımg w white noise")


# açılma
opening = cv2.morphologyEx(noise_img.astype(np.float32), cv2.MORPH_OPEN, kernel)
plt.figure(), plt.imshow(opening, cmap = "gray"), plt. axis("off"), plt.title("açılma IMG")
...
plt.figure(), plt.imshow(black_noise_img, cmap = "gray"), plt. axis("off"), plt.title("img w black noise")

# kapatma
closing = cv2.morphologyEx(black_noise_img.astype(np.float32), cv2.MORPH_CLOSE, kernel)
plt.figure(), plt.imshow(closing, cmap = "gray"), plt. axis("off"), plt.title("kapatma IMG")
